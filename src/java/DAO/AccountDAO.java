...
public class AccountDAO extends DAO {
    ...
    public Account findAccByEmail(String email) {
        String sql = "select * from Account_HE170027 where email = ?";
    ...
    public Account addAcc(String account, String password, String customer, String phone, String address, String email) {
        String sql = "insert into Account_HE170027(account_name, password, customer_name, phone, address, email) values (?,?,?,?,?,?)";
    ...
    public Account getOneAcc(int id) {
        String sql = "Select * from Account_HE170027 where account_id = ?";
        Account acc = new Account();
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setInt(1, id);
    ...
    public List<Account> getAllAccount() {
        String sql = "Select * From Account_HE170027";
    ...
    public int cusAccountExist(String email, String p) {
        String sql = "select a.account_id, a.email, a.password FROM Account_HE170027 as a "
    ...
    public Account findAccByName(String name, String p) {
        String sql = "select * from Account_HE170027 where account_name = ? and password = ?";
        Account acc = new Account();
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setString(1, name);
    ...
    public Account checkAccountExistByUserPass(String user, String pass) {
        try {
            String sql = "SELECT * FROM Account_HE170027 where email = ? and [password] = ?";
    ...
    public void UpDatePassWord(String pass, String user) {
        try {
            String sql = "UPDATE [Account_HE170027]\n"
                    + "   SET [password] = ?\n"
                    + " WHERE [email] = ?";
            PreparedStatement stm = con.prepareStatement(sql);
    ...
    public void updateAccount(String AccountId, String account_name, String customer_name, String phone, String address) {
        try {
            String sql = "UPDATE [dbo].[Account_HE170027]\n"
                    + "   SET [account_name] = ?\n"
                    + "      ,[customer_name] = ?\n"
                    + "      ,[phone] = ?\n"
                    + "      ,[address] = ?\n"
