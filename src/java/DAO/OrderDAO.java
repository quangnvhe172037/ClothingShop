...
public class OrderDAO extends DAO {
    ...
    public boolean addOrder(int aid, List<Cart> carts) {
        ...
        String sql = "INSERT INTO [dbo].[Order_HE170027]\n"
                + "           ([order_date]\n"
                + "           ,[account_id]\n"
                + "           ,[total])\n"
                + "     VALUES\n"
    ...
    public boolean addOrder(int aid, Cart c) {
        String sql = "insert into Order_HE170027 values (?,?,?)";
        Date date = new Date();
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setDate(1, new java.sql.Date(date.getTime()));
    ...
    public int getNewestOrderId() {
        String sql = "select top 1 * from [Order_HE170027] order by order_id desc";
        int id = 1;
        try {
            PreparedStatement ps = con.prepareStatement(sql);
    ...
    public boolean addOrderDetail(List<Cart> carts) {
        String sql = "INSERT INTO [dbo].[OrderDetail_HE170027]\n"
                + "           ([order_id]\n"
                + "           ,[product_id]\n"
                + "           ,[quantity]\n"
                + "           ,[price])\n"
    ...
    public boolean addOrderDetail(Cart c) {
        String sql = "insert into OrderDetail_HE170027 values (?,?,?,?)";
        try {
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setInt(1, getNewestOrderId());
            ps.setInt(2, c.getProductId());
